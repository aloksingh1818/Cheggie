...
export function DeepSeek() {
  ...
  const [chatHistory, setChatHistory] = useState<{ id: string; title: string; timestamp: number }[]>(() => {
    const savedHistory = localStorage.getItem('deepseek-chat-sessions');
    return savedHistory ? JSON.parse(savedHistory) : [];
  });
  const [currentChatId, setCurrentChatId] = useState<string>(() => {
    const savedId = localStorage.getItem('deepseek-current-chat');
  ...
  useEffect(() => {
    localStorage.setItem('deepseek-chat-sessions', JSON.stringify(chatHistory));
    localStorage.setItem('deepseek-current-chat', currentChatId);
  }, [chatHistory, currentChatId]);

  // Save messages for current chat
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem(`deepseek-chat-${currentChatId}`, JSON.stringify(messages));
    }
  ...
  const createNewChat = () => {
    ...
    setSelectedImage(null);
    setImagePreview(null);
    localStorage.setItem('deepseek-chat-history', JSON.stringify([]));
  };

  const switchChat = (chatId: string) => {
    setCurrentChatId(chatId);
    const savedMessages = localStorage.getItem(`deepseek-chat-${chatId}`);
